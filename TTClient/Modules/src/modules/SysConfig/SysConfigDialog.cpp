...
SysConfigDialog::SysConfigDialog() : m_pbtnApply(0) {}
...
void SysConfigDialog::_InitlizeSetting() {
  ...
  if (!pTTConfig) {
    LOG__(ERR, _T("_InitlizeSetting,初始化配置失败"));
    return;
  }
  if (pTTConfig->sysBaseFlag & module::BASE_FLAG_TOPMOST) {
    m_topmostBtn->SetCheck(true);
  }
  if (pTTConfig->sysBaseFlag & module::BASE_FLAG_NOTIPWHENNEWMSG)  // ui初始化时不需要提示
  {
...
void SysConfigDialog::_ApplySetting() {
  ...
  int baseFlag = 0;
  if (!pTTConfig) {
    LOG__(ERR, _T("SysConfigDialog::_ApplySetting,"));
    return;
  }
  ...
  module::getSysConfigModule()->asynNotifyObserver(module::KEY_SYSCONFIG_UPDATED);  // 下一页?
