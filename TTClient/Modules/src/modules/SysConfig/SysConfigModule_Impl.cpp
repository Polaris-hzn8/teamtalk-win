...
#include <modules/SysConfig/SysConfigModule_Impl.h>
...
void SysConfigModule_Impl::showSysConfigDialog(HWND hParentWnd) {
  ...
  pSysConfigDialog->Create(
    hParentWnd, _T("SysConfigDialog"), UI_CLASSSTYLE_DIALOG, WS_EX_STATICEDGE | WS_EX_APPWINDOW, 0, 0, 0, 0);
...
BOOL SysConfigModule_Impl::showServerConfigDialog(HWND hParentWnd) {
  ...
  pServerConfigDialog->Create(
    hParentWnd, _T("服务器配置"), UI_CLASSSTYLE_DIALOG, WS_EX_STATICEDGE | WS_EX_APPWINDOW, 0, 0, 0, 0);
...
BOOL SysConfigModule_Impl::getImage(IN std::string sid,
                                    IN std::string url,
                                    IN BOOL bGrayScale,
  ...
  if (!imageEntity.filename.empty() && PathFileExists(csLocalPath)) { /* 本地磁盘已存在 */
    ...
    return TRUE;
           ...
           //	format = "_60x60";
           // }
           // DownloadImgHttpOperation* pOper = new DownloadImgHttpOperation(sid, url, bGrayScale, format,
           // BIND_CALLBACK_1(callback)); module::getHttpPoolModule()->pushHttpOperation(pOper);
  }
  return FALSE;
}
