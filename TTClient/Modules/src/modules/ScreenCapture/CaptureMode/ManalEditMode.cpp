...
ManalEditMode::ManalEditMode() : m_iSelectIdx(-1) {
...
void ManalEditMode::onLButtonDown(__in int x, __in int y) {
  UIScreenCaptureMgr::Instance()->sendEditToolbarMessage(
    WM_SNAPSHOT_TEST_SELECT_IDX, (WPARAM)&m_iSelectIdx, (LPARAM)&y);
...
void ManalEditMode::onMouseMove(__in int x, __in int y) {
  ...
  if (0 == m_iSelectIdx)  // ¾ØÐÎ
  {
    onRectangle(x, y);
  } else if (1 == m_iSelectIdx)  // ÍÖÔ²
  {
    onPolyRegion(x, y);
  } else if (2 == m_iSelectIdx)  // ×ÔÓÉ»­
  {
    onFreeRegion(x, y);
  }
}
