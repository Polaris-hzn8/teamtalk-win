...
#include <modules/TcpClient/TcpClientModule_Impl.h>
...
IM::Login::IMLoginRes* TcpClientModule_Impl::doLogin(CString& linkaddr,
                                                     UInt16 port,
                                                     CString& uName,
                                                     std::string& pass) {
...
void TcpClientModule_Impl::sendPacket(UInt16 moduleId,
                                      UInt16 cmdId,
                                      UInt16 seq,
                                      google::protobuf::MessageLite* pbBody) {
  m_TTPBHeader.clear();
  m_TTPBHeader.setModuleId(moduleId);
  m_TTPBHeader.setCommandId(cmdId);
...
ServerPingTimer::ServerPingTimer(TcpClientModule_Impl* pTcpClient)
  : m_pTcpClient(pTcpClient), m_bHasReceivedPing(FALSE) {}
