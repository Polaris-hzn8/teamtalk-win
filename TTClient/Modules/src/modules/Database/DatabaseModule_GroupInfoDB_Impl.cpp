...
const std::string createGroupInfoSql =
  ...
  "    [type]							INTEGER DEFAULT 2,"  // 6.group type,1:讨论组 2:普通群
  "    [version]						INTEGER DEFAULT 0,"  // 7.group info version
  "    [lastUpdateTime]				INTEGER DEFAULT 0,"                  // 8.last update time
  "    [shieldStatus]					INTEGER DEFAULT 0,"  // 9.群消息屏蔽状态 0: 未屏蔽 1: 已屏蔽
  "    [memberlist]					TEXT NOT NULL,"  // 10.group member list which storage by json
                                                                         // format
  "    [reserve1]                     TEXT DEFAULT NULL,"
  "    [reserve2]                     INTEGER DEFAULT NULL,"
  "    [reserve3]                     INTEGER DEFAULT NULL"
  ");";
const std::string createGroupInfoIndex = "create unique index if not exists groupid_idx ON groupinfo(groupId);";
const std::string getAllGroupInfoSql = "select * from groupinfo limit 10000";
const std::string getGroupInfoByGIdSql = "select * from groupinfo where groupId=? limit 1";
const std::string insertGroupInfoSql =
  "INSERT OR REPLACE into "
  "groupinfo(groupId,name,desc,avatarUrl,creatorId,type,version,lastUpdateTime,shieldStatus,memberlist) "
  "VALUES(?,?,?,?,?,?,?,?,?,?);";
const std::string deleteGroupInfoSql = "delete from groupinfo where groupId=?";
const std::string updateGroupInfoBySIdSql =
  "update groupinfo set "
  "name=?,desc=?,avatarUrl=?,creatorId=?,type=?,version=?,lastUpdateTime=?,"
  "shieldStatus=?,memberlist=? where groupId=?";
const std::string BeginInsert = "BEGIN TRANSACTION;";
const std::string EndInsert = "COMMIT TRANSACTION;";
...
#endif
    CString csErrMsg = util::stringToCString(sqliteException.errorMessage(), CP_UTF8);
    LOG__(ERR, _T("select failed,error msg:%s"), csErrMsg);
    return FALSE;
  } catch (...) {
    LOG__(ERR, _T("db unknown exception"));
    return FALSE;
...
BOOL DatabaseModule_Impl::sqlUpdateGroupInfoEntity(std::string& sId, IN const module::GroupInfoEntity& groupInfo) {
  try {
    //"update groupinfo set
    // name=?,desc=?,avatarUrl=?,creatorId=?,type=?,version=?,lastUpdateTime=?,shieldStatus=?,memberlist=? where
    // groupId=?";
...
BOOL DatabaseModule_Impl::_execGroupInfoCreateTableDML() {
  try {
    ...
    CString csErrMsg = util::stringToCString(sqliteException.errorMessage(), CP_UTF8);
    LOG__(ERR, _T("create table failed,error msg:%s"), csErrMsg);
#ifdef _DEBUG
    MessageBoxA(0, sqliteException.errorMessage(), "BD ERROR", MB_OK | MB_ICONHAND);
#endif
    return FALSE;
...
void DatabaseModule_Impl::_parseJsonForGroupMembers(IN std::string strJson,
                                                    OUT std::list<std::string>& groupMemeberList) {
  Json::Reader reader;
  Json::Value root;
