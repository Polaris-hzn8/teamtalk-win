...
LoginOperation::LoginOperation(module::IOperationDelegate callback, LoginParam& param)
  : ICallbackOpertaion(callback), m_loginParam(param) {}
...
void LoginOperation::processOpertion() {
  ...
  IM::Login::IMLoginRes* pImLoginResp = (IM::Login::IMLoginRes*)module::getTcpClientModule()->doLogin(
    msgServerIp, msgServerPort, m_loginParam.csUserName, m_loginParam.password);

  if (pImLoginResp == NULL || pImLoginResp->result_code() != IM::BaseDefine::REFUSE_REASON_NONE ||
      !pImLoginResp->has_user_info()) {
    // 登录失败使用备用ip
    LOG__(
      ERR, _T("add:%s:%d, uname:%s,login for msg server failed"), msgServerIp, msgServerPort, m_loginParam.csUserName);
    if (pImLoginResp) {
  ...
  LOG__(APP,
        _T("login succeed! Name = %s Nickname = %s sId = %s status = %d"),
