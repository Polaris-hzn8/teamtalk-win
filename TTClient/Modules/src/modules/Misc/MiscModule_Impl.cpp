...
CString MiscModule_Impl::makeShortTimeDescription(IN UInt32 Intime) {
  ...
  if (Intime >
      static_cast<UInt32>(time(0)) + 86400)  // 居然比现在还早 86400为一天，服务器时间可能和本地时间有误差，故加一天时差
  {
...
void MiscModule_Impl::doProcess1(IN std::string& pData, OUT std::string& pOutData) {
  LOG__(DEBG, _T("EncryptMsg, pData content:%s"), util::stringToCString(pData));
  if (pData.empty())
    return;
  ...
  if (retCode == 0 && nOutLen > 0 && pOutDataTemp != 0) {
    ...
    LOG__(ERR, _T("EncryptMsg Failed!,msg content:%s"), util::stringToCString(pData));
...
void MiscModule_Impl::doProcess2(IN std::string& pData, OUT std::string& pOutData) {
  ...
  if (retCode == 0 && nOutLen > 0 && pOutDataTemp != 0) {
    ...
    LOG__(ERR, _T("DecryptMsg Failed!,pData content:%s"), util::stringToCString(pData));
    // if failed, copy the source msg data
    pOutData = pData;
  }
}