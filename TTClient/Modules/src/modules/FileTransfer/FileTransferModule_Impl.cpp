...
FileTransferModule_Impl::FileTransferModule_Impl() : m_pFileTransferDialog(0) {
...
void FileTransferModule_Impl::MKOForFileTransferModuleCallback(const std::string& keyId, MKO_TUPLE_PARAM mkoParam) {
  if (module::KEY_FILETRANSFER_REQUEST == keyId || module::KEY_FILETRANSFER_SENDFILE == keyId) {
    if (m_pFileTransferDialog) {
...
BOOL FileTransferModule_Impl::sendFile(IN const CString& sFilePath,
                                       IN const std::string& sSendToSID,
                                       IN BOOL bOnlineMode) {
  ...
  if (0 != fileEntity.nFileSize) {
    ...
    transMode = bOnlineMode ? IM::BaseDefine::TransferFileType::FILE_TYPE_ONLINE
                            : IM::BaseDefine::TransferFileType::FILE_TYPE_OFFLINE;
    ...
    imcore::IMLibCoreStartOperationWithLambda([=]() {
      ...
      LOG__(APP,
            ...
            util::stringToCString(fileEntity.sFileName),
            fileEntity.nFileSize,
            util::stringToCString(fileEntity.sToID));
      ...
      module::getTcpClientModule()->sendPacket(
        IM::BaseDefine::ServiceID::SID_FILE, IM::BaseDefine::FileCmdID::CID_FILE_REQUEST, &imFileReq);
    });
...
void FileTransferModule_Impl::_sendfileResponse(IN std::string& body) {
  ...
  LOG__(
    DEBG, _T("FileTransferSevice_Impl::准备连接文件服务器 sTaskId = %s"), util::stringToCString(fileEntity.sTaskID));
...
BOOL FileTransferModule_Impl::acceptFileTransfer(IN const std::string& sTaskId, IN const BOOL bAccept /*= TRUE*/) {
  ...
  if (TransferFileEntityManager::getInstance()->getFileInfoByTaskId(sTaskId, FileInfo)) {
    if (bAccept)  // 接收文件
    {
      ...
      LOG__(DEBG,
            _T("FileTransferSevice_Impl::acceptFileTransfer 接收文件 sFileID = %s"),
    ...
    } else  // 拒绝文件
    {
      ...
      {
        LOG__(DEBG,
              _T("FileTransferSevice_Impl::acceptFileTransfer 拒绝文件 sFileID = %s"),
              util::stringToCString(FileInfo.sTaskID));
