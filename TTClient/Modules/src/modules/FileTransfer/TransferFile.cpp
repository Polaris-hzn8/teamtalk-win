...
TransferFile::FilePrivate::FilePrivate(const CString& path, BOOL bsave) : file(0), readOnly(TRUE) {
  if (!path.IsEmpty()) {
    if (bsave) {
...
TransferFile::TransferFile(const CString& path, BOOL bsave) : m_pfileObject(nullptr) {
