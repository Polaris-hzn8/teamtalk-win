...
void FileTransferSocket::_filePullDataReqResponse(IN std::string& body)  // 回复拉取数据请求
{
  ...
  LOG__(DEBG,
        _T("send:taskId=%s,filesize=%d,name=%s,BolckSize=%d"),
  ...
  // send packet
  sendPacket(
    IM::BaseDefine::ServiceID::SID_FILE, IM::BaseDefine::FileCmdID::CID_FILE_PULL_DATA_RSP, &imFilePullDataRsp);
  ...
  if (fileEntity.nProgress < fileEntity.nFileSize) {
    ...
    module::getFileTransferModule()->asynNotifyObserver(module::KEY_FILESEVER_UPDATA_PROGRESSBAR, fileEntity.sTaskID);
  } else  // 传输完成
  {
...
void FileTransferSocket::_filePullDataRspResponse(IN std::string& body)  // 处理拉取数据响应
{
  ...
  LOG__(DEBG,
        _T("receive:taskId=%s,filesize=%d,name=%s,BolckSize=%d"),
        util::stringToCString(fileEntity.sTaskID),
        fileEntity.nFileSize,
        fileEntity.getRealFileName(),
        nBlockSize);
...
PingFileSevTimer::PingFileSevTimer(FileTransferSocket* pTransSocket) : m_pFileTransSocket(pTransSocket) {}
