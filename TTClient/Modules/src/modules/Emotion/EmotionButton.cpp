...
CEmotionButton::CEmotionButton(void) {
  ...
  CELLSIZE = 29;  //???????= 29*30???????????24*24??

  CELLCOUNT_LINE = 15;   //???
  CELLCOUNT_COLUMN = 8;  //???
  CELLCOUNT_PAGE = 120;  //??120??????
...
void CEmotionButton::DoEvent(TEventUI& event) {
  switch (event.Type) {
    case UIEVENT_MOUSEMOVE: {
      ...
      unsigned char changetype = _PosChangeType::NoChange;  //????????????????
      ...
      if (tmpSel != curSel) {
        ...
        if (curSel >= 0) {
          ...
          // QQ??????????????10????????????????????50????
          if (pFrame != NULL)
            frameDelay = max(frameDelay, pFrame->GetFrameDelay());
      ...
      //????????? WM_MOUSELEAVE;
      TrackMouseEvent(&m_tme);  //???��????_WIN32_WINNT 0x0510
    } break;
    ...
    case UIEVENT_BUTTONUP: {
      ...
      int index = curPage * CELLCOUNT_PAGE + curSel;  //???????????????????????????
      if (index > -1 && index < (int)m_mapImagePath.size()) {
        ...
        module::getEmotionModule()->asynNotifyObserver(module::KEY_EMOTION_SELECTED,
                                                       std::shared_ptr<void>(pEmotionParam));
...
void CEmotionButton::SwitchPage(int curPage) {
  ...
  //?????????????��???
  //??????????????
  for (index = (curPage * CELLCOUNT_PAGE), count = 0; (index < m_ImageCount && count < CELLCOUNT_PAGE);
       index++, count++) {
    //???��???
...
void CEmotionButton::UpdateSelectedFace(int curPage, int curSel, int curFrame, int pvstatus) {
  ...
  if (lpRect != NULL) {
    index = curPage * CELLCOUNT_PAGE + curSel;         //???????????????????????????
    hPen2 = CreatePen(PS_SOLID, 1, RGB(0, 138, 255));  //?????????
    hTempPen = SelectObject(m_hMemDC, hPen2);
    hTempBrush = SelectObject(m_hMemDC, GetStockObject(WHITE_BRUSH));  //??????
