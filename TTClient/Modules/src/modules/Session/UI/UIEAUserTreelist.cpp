...
CEAUserTreelistUI::CEAUserTreelistUI(CPaintManagerUI& paint_manager) : UIIMList(paint_manager) {}
...
Node* CEAUserTreelistUI::AddNode(const EAUserTreeListItemInfo& item, Node* parent) {
  ...
  if (!m_dlgBuilder.GetMarkup()->IsValid()) {
    pListElement = static_cast<CListContainerElementUI*>(
      m_dlgBuilder.Create(_T("MainDialog\\eaUserListItem.xml"), (UINT)0, NULL, &paint_manager_));
  } else {
