...
bool UIIMList::Remove(CControlUI* pControl) {
  ...
  if (reinterpret_cast<Node*>(
        static_cast<CListContainerElementUI*>(pControl->GetInterface(_T("ListContainerElement")))->GetTag()) == NULL)
    return CListUI::Remove(pControl);
  else
    return RemoveNode(reinterpret_cast<Node*>(
      static_cast<CListContainerElementUI*>(pControl->GetInterface(_T("ListContainerElement")))->GetTag()));
}
  ...
  if (!m_dlgBuilder.GetMarkup()->IsValid()) {
    if (!m_IitemXmlFilePath.IsEmpty()) {
      pListElement = static_cast<CListContainerElementUI*>(
        m_dlgBuilder.Create(STRINGorID(m_IitemXmlFilePath), (UINT)0, NULL, &paint_manager_));
    } else
      pListElement = static_cast<CListContainerElementUI*>(
...
void UIIMList::SetChildVisible(Node* node, bool visible) {
  ...
  if (node->data().has_child_) {
    ...
    CLabelUI* nick_name =
      static_cast<CLabelUI*>(paint_manager_.FindSubControlByName(node->data().list_elment_, kNickNameControlName));
    if (nick_name != NULL) {
      nick_name->SetShowHtml(true);
      nick_name->SetText(html_text);
    }
  ...
  for (int i = begin->data().list_elment_->GetIndex(); i <= end->data().list_elment_->GetIndex(); ++i) {
    ...
    if (_tcsicmp(control->GetClass(), _T("ListContainerElementUI")) == 0) {
      if (visible) {
        ...
        if (local_parent->data().child_visible_ && local_parent->data().list_elment_ &&
...
void UIIMList::_freshAvatarBySId(std::shared_ptr<void> pParam) {
  ...
  if (strModule == module::MODULE_USERLIST_PREFIX) {
    ...
    // 更新下名字的字体颜色
    CLabelUI* nick_name =
      static_cast<CLabelUI*>(paint_manager_.FindSubControlByName(pListElement, kNickNameControlName));
    PTR_VOID(nick_name);
