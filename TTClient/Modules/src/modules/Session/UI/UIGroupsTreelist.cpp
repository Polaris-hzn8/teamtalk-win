...
CGroupsTreelistUI::CGroupsTreelistUI(CPaintManagerUI& paint_manager) : UIIMList(paint_manager) {}
...
Node* CGroupsTreelistUI::AddNode(const GroupsListItemInfo& item, Node* parent) {
  ...
  if (!UIIMList::m_dlgBuilder.GetMarkup()->IsValid()) {
    pListElement = static_cast<CListContainerElementUI*>(
      UIIMList::m_dlgBuilder.Create(kXMLFilePath, (UINT)0, NULL, &paint_manager_));
  } else {
  ...
  CControlUI* log_button =
    static_cast<CControlUI*>(UIIMList::paint_manager_.FindSubControlByName(pListElement, kLogoButtonControlName));
  if (log_button != NULL) {
  ...
  CLabelUI* nick_name =
    static_cast<CLabelUI*>(UIIMList::paint_manager_.FindSubControlByName(pListElement, kNickNameControlName));
  if (nick_name != NULL) {
    if (item.folder)
  ...
  if (item.folder && item.id == MY_DISCUSSGROUP_ITEMID) {
    CButtonUI* btnCreatGroup =
      static_cast<CButtonUI*>(UIIMList::paint_manager_.FindSubControlByName(pListElement, kcreatGroupControlName));
