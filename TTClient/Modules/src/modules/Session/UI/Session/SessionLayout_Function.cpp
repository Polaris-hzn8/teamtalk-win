...
void SessionLayout::_SendSessionMsg(IN MixedMsg mixMsg) {
  if (mixMsg.IsPureTextMsg()) {
    ...
    for (ST_picData& picData : mixMsg.m_picDataVec) {
      ...
      m_pSendImgHttpOper = new SendImgHttpOperation(param, BIND_CALLBACK_1(SessionLayout::OnSendImageCallback));
...
void SessionLayout::SendMsg() {
  ...
  if (module::TCPCLIENT_STATE_OK == netState && IM::BaseDefine::USER_STATUS_OFFLINE != myInfo.onlineState) {
    MixedMsg mixMsg;
