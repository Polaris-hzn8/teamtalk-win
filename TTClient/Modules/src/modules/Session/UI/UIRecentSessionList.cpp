...
CUIRecentSessionList::CUIRecentSessionList(CPaintManagerUI& paint_manager) : UIIMList(paint_manager) {}
...
Node* CUIRecentSessionList::AddNode(const SessionListItemInfo& item, Node* parent /*= NULL*/, int index /*= 0*/) {
  ...
  if (!m_dlgBuilder.GetMarkup()->IsValid()) {
    pListElement = static_cast<CListContainerElementUI*>(
      m_dlgBuilder.Create(_T("MainDialog\\recentlyListItem.xml"), (UINT)0, NULL, &paint_manager_));
  } else {
  ...
  CControlUI* log_button =
    static_cast<CControlUI*>(paint_manager_.FindSubControlByName(pListElement, kLogoButtonControlName));
  if (log_button != NULL) {
    if (!item.folder && !item.avatarPath.IsEmpty()) {
      _stprintf_s(szBuf, MAX_PATH - 1, _T("%s"), item.avatarPath);
      log_button->SetBkImage(szBuf);
    } else {
      CContainerUI* logo_container =
        static_cast<CContainerUI*>(paint_manager_.FindSubControlByName(pListElement, kLogoContainerControlName));
  ...
  CLabelUI* lastTime =
    static_cast<CLabelUI*>(paint_manager_.FindSubControlByName(pListElement, lastContentTimeControlName));
  if (lastTime != NULL) {
    lastTime->SetShowHtml(true);
    CString strTime =
      module::getMiscModule()->makeShortTimeDescription(item.Time);  // timeData.Format(_T("-%Y%m%d-%H-%M-%S-"));
...
BOOL CUIRecentSessionList::UpdateItemConentBySId(IN const std::string& sId) {
  ...
  CLabelUI* Unreadcnt_button =
    static_cast<CLabelUI*>(paint_manager_.FindSubControlByName(pListElement, kUnreadcntControlName));
  if (!Unreadcnt_button) {
    return FALSE;
  }
...
BOOL CUIRecentSessionList::UpdateItemInfo(IN const SessionListItemInfo& item) {
  ...
  CControlUI* pListElement = pNode->data().list_elment_;
  PTR_FALSE(pListElement);
  CContainerUI* logo_Button =
    static_cast<CContainerUI*>(paint_manager_.FindSubControlByName(pListElement, kLogoButtonControlName));
  if (logo_Button != NULL)
