...
#include <modules/Session/SessionModule_Impl.h>
...
DuiLib::CControlUI* SessionModule_Impl::createMainDialogControl(IN LPCTSTR pstrClass,
                                                                IN DuiLib::CPaintManagerUI& paintManager) {
  ...
  if (groupInfo.shieldStatus) {
    ...
    network::IMLibCoreStartOperationWithLambda([=]() mutable {
      ...
      module::getTcpClientModule()->sendPacket(
        IM::BaseDefine::ServiceID::SID_MSG, IM::BaseDefine::MessageCmdID::CID_MSG_READ_ACK, &imMsgDataReadAck);
    });
    return TRUE;
  }
  return FALSE;
...
BOOL SessionModule_Impl::_prase2LocalMsg(OUT MessageEntity& msg) {
  if (IM::BaseDefine::MsgType::MSG_TYPE_SINGLE_AUDIO == msg.msgType) {
    // 实时个人消息需要多端同步
    if (MESSAGE_TYPE_RUNTIME == msg.msgStatusType && msg.talkerSid != module::getSysConfigModule()->userID())
    ...
    AudioMessageMananger::getInstance()->saveAudioDataToFile(
      (UCHAR*)(msg.content.c_str()), msg.content.length(), fileName);
    AudioMessageMananger::getInstance()->getAudioMsgLenth(
      (UCHAR*)(msg.content.c_str()), msg.content.length(), msg.msgAudioTime);
  ...
  if (ReceiveMsgManage::getInstance()->pushMessageBySId(msg.sessionId, msg)) {
    ...
    if (MESSAGETYPE_FROM_FRIEND == msg.msgSessionType)  // 只有个人消息要发已收确认,群消息是不发已收确认的
    {
      network::IMLibCoreStartOperationWithLambda([=]() {
        ...
        module::getTcpClientModule()->sendPacket(
          IM::BaseDefine::ServiceID::SID_MSG, IM::BaseDefine::MessageCmdID::CID_MSG_DATA_ACK, &imMsgDataAck);
      });
    }
  }
}
void SessionModule_Impl::_sessionUnReadMsgListResponse(IN std::string& pbBody) {
  ...
  LOG__(
    APP, _T("IMGetMsgListRsp msgCnt：%d,sessionId =%s"), msgCnt, util::int32ToCString(imGetMsgListRsp.session_id()));
...
void SessionModule_Impl::_sessionHistoryMsgListResponse(IN UInt16 reserved, IN std::string& pbBody) {
  ...
  const UInt32 msgCnt = imGetMsgListRsp.msg_list_size();
  LOG__(APP,
        _T("IMGetMsgListRsp log begin,sessionid：%d,last msgId:%d,msg count:%d,sessionType:%d"),
        imGetMsgListRsp.session_id(),
        imGetMsgListRsp.msg_id_begin(),
        msgCnt,
        imGetMsgListRsp.session_type());
  ...
  for (int i = 0; i < nSize; ++i) {
    ...
    network::IMLibCoreStartOperationWithLambda([=]() {
      LOG__(APP,
            ...
            static_cast<UInt32>(unReadInfo.session_type()));
      ...
      if (nUnReadCnt > 100) {  // 当未读消息大于100条,只读取最近的100条，否则会导致UI刷新问题
...
void SessionModule_Impl::updateSessionEntityByMsg(IN const MessageEntity& msg) {
  ...
  if (MESSAGE_RENDERTYPE_AUDIO == msg.msgRenderType) {
    std::string sYuYing =
      util::cStringToString(util::getMultilingual()->getStringById(_T("STRID_SESSIONMODULE_RENDERTYPE_AUDIO")));
    std::string msgEncrypt;
  ...
  module::getSessionModule()->asynNotifyObserver(module::KEY_SESSION_UPDATE_RECENTLY_SESSIONLIST,
