...
SessionDialog* SessionDialogManager::openSessionDialog(IN const std::string& sId) {
  if (sId.empty())
    return 0;
  auto iterDialog = std::find_if(m_lstSessionDialog.begin(), m_lstSessionDialog.end(), [=](SessionDialog* pDialog) {
...
void SessionDialogManager::removeSessionDialogPtr(IN const std::string& sId) {
  auto iterDialog = std::remove_if(m_lstSessionDialog.begin(), m_lstSessionDialog.end(), [=](SessionDialog* pDialog) {
    return (sId == pDialog->m_sId);
  });
  if (iterDialog != m_lstSessionDialog.end()) {
