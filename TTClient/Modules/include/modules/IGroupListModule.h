...
// KEYID
const std::string KEY_GROUPLIST_UPDATE_GROUPLIST = MODULE_GROUPLIST_PREFIX + "GroupList";  // UI更新固定群列表事件
const std::string KEY_GROUPLIST_UPDATE_GROUPINFO_UPDATE =
  MODULE_GROUPLIST_PREFIX + "GroupInfoUpdate";  // 更新群信息（可能是固定群也可能是讨论组）
const std::string KEY_GROUPLIST_UPDATE_CREATNEWGROUP = MODULE_GROUPLIST_PREFIX + "CreateNewGroup";   // 创建新的群返回
const std::string KEY_GROUPLIST_UPDATE_MEMBER_CHANGED = MODULE_GROUPLIST_PREFIX + "MembersChanged";  // 成员变动
const std::string KEY_GROUPLIST_UPDATE_MYSELF_KICKED = MODULE_GROUPLIST_PREFIX + "MySelfKicked";     // 自己被踢出群
...
class GroupInfoEntity {
 public:
  ...
  CString desc;                 // 群描述
  CString csName;
  UInt32 type = 0;          // 群类型：1为固定群 2,讨论组
  UInt32 groupUpdated = 0;  // 最近一次更新信息时间
  UInt32 version = 0;       // 群版本
  UInt32 shieldStatus = 0;  // 群消息提醒是否开启 0: 免打扰 1: 提醒
...
typedef std::vector<std::string> GroupVec;                    // 群列表ID,包括讨论组群
/**
 * The class <code>群信息管理模块</code>
 *
 */
class MODULE_API IGroupListModule : public module::ModuleBase, public module::IPduPacketParse {
 public:
  /**
   * 获取所有群列表信息，用于不在群列表
   *
   ...
   * @exception there is no any exception to throw.
   */
  virtual void tcpGetGroupInfo(IN const std::string& groupId) = 0;                         // 获取群的群信息
  virtual void tcpGetGroupsInfo(IN const module::GroupVec& VecGroupId) = 0;                // 获取多个群信息
  virtual void tcpShieldGroup(IN const std::string& groupId, IN UInt32 shieldStatus) = 0;  // 设置群消息提醒状态
