...
//KEYID
const std::string KEY_SESSION_NEWMESSAGE		= MODULE_SESSION_PREFIX + "NewMessage";			//收到新消息（包含离线消息、离线消息推送）
...
const std::string KEY_SESSION_TRAY_NEWMSGSEND	= MODULE_SESSION_PREFIX + "TrayNewMsgSend"; 	//发送一条新消息到托盘的提示框
...
const std::string KEY_SESSION_MESSGEREADED_NOTIFY = MODULE_SESSION_PREFIX + "MessageReadEdNotify";//有人阅读了发出去的消息
const std::string KEY_SESSION_UPDATE_RECENTLY_SESSIONLIST = MODULE_SESSION_PREFIX + "UpdateRecentlySessionList";		//更新最近会话列表
const std::string KEY_SESSION_UPDATE_TOTAL_UNREADMSG_COUNT = MODULE_SESSION_PREFIX + "UpdateTotalUnReadMsgCount";		//更新未读消息数
...
public:
	...
	virtual UInt32 getGlobalUpdateTime()const = 0;//从服务器端获取最近更新的会话列表，需要提交给服务器一次更新会话的时间
