...
TransferFileEntity::TransferFileEntity() : nFileSize(0), nClientMode(0), nProgress(0), pFileObject(nullptr) {
  sSavePath.Empty();
}
...
TransferFileEntityManager::TransferFileEntityManager() : m_fileUIThread(0) {}
...
BOOL TransferFileEntityManager::openFileByFileID(IN const std::string& sID) {
  std::vector<TransferFileEntity>::iterator it =
    m_VecFinishedFile.begin();  // 已经传输完的文件,任何操作过的文件都会被扔到这里
    ...
    if (sID == it->sTaskID) {
      ...
      if (PathFileExists(strPath)) {
        ...
        if ((long)hr < 32) {
          ...
          LOG__(ERR, _T("ShellExecute Failed GetLastError = %d"), dwLastError);
        }
      }
      return TRUE;
    }
  ...
  for (; it != m_VecFinishedFile.end(); ++it) {
    if (sID == it->sTaskID) {
      ...
      if (TRUE)  // 判断文件夹能不能找到
      {
        ...
        HINSTANCE hr =
          ShellExecute(NULL, _T("open"), _T("Explorer.exe"), _T("/select,") + strPath, NULL, SW_SHOWDEFAULT);
        if ((long)hr < 32) {
          DWORD dwLastError = ::GetLastError();
          LOG__(ERR, _T("ShellExecute Failed GetLastError = %d"), dwLastError);
        }
...
BOOL TransferFileEntityManager::checkIfIsSending(IN CString sFilePath) {
  ...
  for (; it != m_MapFile.end(); ++it) {
    ...
    if ((IM::BaseDefine::ClientFileRole::CLIENT_OFFLINE_UPLOAD == it->second.nClientMode ||
         IM::BaseDefine::ClientFileRole::CLIENT_REALTIME_SENDER == it->second.nClientMode) &&
        sfileName == sFilePath) {
