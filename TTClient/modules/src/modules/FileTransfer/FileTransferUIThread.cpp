...
namespace {
const UInt32 FILE_TRANSFER_BLOCK_SIZE = 1024 * 34;  // 文件传输块大小
}

FileTransferUIThread::FileTransferUIThread() : m_hWnd(nullptr) {}
...
HWND FileTransferUIThread::_createWnd() {
  ...
  hwnd = ::CreateWindowW(
    _T("filetransfer"), _T("filetransferWindow"), 0, 0, 0, 0, 0, HWND_MESSAGE, 0, GetModuleHandle(0), 0);
    ...
    imcore::IMLibCoreStartOperationWithLambda([=]() {
      ...
      if (TransferFileEntityManager::getInstance()->getFileInfoByTaskId(taskId, fileEntity)) {
        int mode = fileEntity.nClientMode == IM::BaseDefine::ClientFileRole::CLIENT_OFFLINE_DOWNLOAD
                     ...
                     : IM::BaseDefine::TransferFileType::FILE_TYPE_ONLINE;
        ...
        fileEntity.nFileSize > FILE_TRANSFER_BLOCK_SIZE ? imFilePullDataReq.set_data_size(FILE_TRANSFER_BLOCK_SIZE)
                                                        : imFilePullDataReq.set_data_size(fileEntity.nFileSize);
        // 接受文件
        pFileSocket->sendPacket(
          IM::BaseDefine::ServiceID::SID_FILE, IM::BaseDefine::FileCmdID::CID_FILE_PULL_DATA_REQ, &imFilePullDataReq);
  ...
  if (pFileSocket) {
    imcore::IMLibCoreStartOperationWithLambda([=]() {
      ...
      if (TransferFileEntityManager::getInstance()->getFileInfoByTaskId(taskId, fileEntity)) {
        ...
        // 拒绝
        pFileSocket->sendPacket(
          IM::BaseDefine::ServiceID::SID_FILE, IM::BaseDefine::FileCmdID::CID_FILE_STATE, &imFileState);
