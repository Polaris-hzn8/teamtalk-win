...
FileTransferDialog::FileTransferDialog() : m_plstFileTransfer(0), m_pprsTransProgress(0) {
  module::getFileTransferModule()->addObserver(this,
                                               BIND_CALLBACK_2(FileTransferDialog::MKOForFileTransferModuleCallBack));
}
...
void FileTransferDialog::OnWindowInitialized(TNotifyUI& msg) {
  ...
  for (std::vector<TransferFileEntity>::reverse_iterator reit = historyfileList.rbegin();
       reit != historyfileList.rend();
       ++reit)  // 倒序输出，历史记录是按时间排的
  {
...
CListContainerElementUI* FileTransferDialog::AddTransderItemToList(IN TransferFileEntity fileInfo) {
  ...
  CListContainerElementUI* pListElement = (CListContainerElementUI*)dlgBuilder.Create(
    _T("FileTransferDialog\\FileTransferListItem.xml"), (UINT)0, NULL, &m_PaintManager);
  if (!pListElement) {
    return NULL;
  }
...
void FileTransferDialog::_UpdateFileTransferState(IN CListContainerElementUI* pListElement,
...
void FileTransferDialog::MKOForFileTransferModuleCallBack(const std::string& keyId, MKO_TUPLE_PARAM mkoParam) {
  if (module::KEY_FILESEVER_UPLOAD_FAILED == keyId || module::KEY_FILESEVER_UPLOAD_REJECT == keyId ||
    ...
    if (IM::BaseDefine::ClientFileRole::CLIENT_OFFLINE_DOWNLOAD == FileInfo.nClientMode) {
      LOG__(APP,
            _T("IMFileDelOfflineReq,name=%s,taskid=%s"),
      ...
      module::getTcpClientModule()->sendPacket(
        IM::BaseDefine::ServiceID::SID_FILE, IM::BaseDefine::FileCmdID::CID_FILE_DEL_OFFLINE_REQ, &imFileDelOfflineReq);
      ...
      module::getTcpClientModule()->sendPacket(
        IM::BaseDefine::ServiceID::SID_FILE, IM::BaseDefine::FileCmdID::CID_FILE_ADD_OFFLINE_REQ, &imFileAddOfflineReq);
    }
    // todo...
    // TransferFileEntityManager::getInstance()->closeFileSocketByTaskId(sFileId);
