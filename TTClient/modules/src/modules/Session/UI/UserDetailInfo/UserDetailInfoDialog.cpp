...
UserDetailInfoDialog::UserDetailInfoDialog(IN std::string sid) : m_sid(sid), m_AvatarUI(nullptr) {}
...
void UserDetailInfoDialog::_refreshAvatar(IN const std::string& avatarPath) {
  ...
  } else  // 不存在则去服务器下载
  {
    ...
    m_pDownHttpOper = new DownloadAvatarHttpOperation(
      m_sid, path, FALSE, AVATAR_FORMAT_170X170, BIND_CALLBACK_1(UserDetailInfoDialog::onHttpGetAvatarCallback));
