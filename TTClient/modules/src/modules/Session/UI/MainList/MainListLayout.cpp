...
LPVOID MainListLayout::GetInterface(LPCTSTR pstrName) {
  if (_tcscmp(pstrName, _T("ListLayout")) == 0)
    return static_cast<MainListLayout*>(this);
...
void MainListLayout::DoInit() {
  ...
  m_UIRecentConnectedList =
    static_cast<CUIRecentSessionList*>(m_pManager->FindSubControlByName(this, _T("recentlyList")));
  ...
  imcore::IMLibCoreStartOperationWithLambda([]() {
    ...
    module::getTcpClientModule()->sendPacket(
      IM::BaseDefine::ServiceID::SID_BUDDY_LIST,
...
void MainListLayout::_LoadAllDepartment() {
  ...
  std::sort(
    vecDepart.begin(), vecDepart.end(), [=](const module::DepartmentEntity& x, const module::DepartmentEntity& y) {
  ...
  for (auto depart : vecDepart) {
    ...
    for (std::string uId : depart.members) {
      ...
      if (module::getUserListModule()->getUserInfoBySId(uId, user)) {
        ...
        LOG__(DEBG,
              _T("can't find the userInfo:%s,GroupID:%s"),
              util::stringToCString(uId),
              util::stringToCString(depart.dId));
      }
    }
  }
}
...
void MainListLayout::_AddGroupList() {
  ...
  item.nickName.Format(util::getMultilingual()->getStringById(_T("STRID_SESSIONMODULE_MAINLISTLAYOUT_GROUP_TIP")),
                       GroupCnt);
...
void MainListLayout::_AddDiscussGroupList() {
  ...
  item.nickName.Format(util::getMultilingual()->getStringById(_T("STRID_SESSIONMODULE_MAINLISTLAYOUT_DISCUSS_TIP")),
                       GroupCnt);
  rootParent = m_GroupList->GetItemBySId(util::cStringToString(CString(item.id)));
  if (!rootParent)
    rootParent = m_GroupList->AddNode(item, NULL);
...
void MainListLayout::_AddRecentSessionListToUI() {
  ...
  for (std::string sessionId : vecRecentSession) {
    ...
    if (IM::BaseDefine::SessionType::SESSION_TYPE_GROUP ==
        static_cast<IM::BaseDefine::SessionType>(sessionEntity.sessionType)) {
      ...
      {
        LOG__(APP,
              _T("Can't get the GroupInfoEntity,Need to get Unkonw GroupInfo:%s"),
              util::stringToCString(sessionEntity.sessionID));
    ...
    } else if (IM::BaseDefine::SessionType::SESSION_TYPE_SINGLE ==
               static_cast<IM::BaseDefine::SessionType>(sessionEntity.sessionType)) {
...
void MainListLayout::_NewGroupsUpdate(std::shared_ptr<void> pGroupIDs) {
  if (nullptr == m_UIRecentConnectedList || nullptr == m_GroupList) {
    return;
  }
...
void MainListLayout::_UpdateRecentSessionItem(IN const std::string& sessionId, IN const UInt32 sessionType) {
  ...
  if (IM::BaseDefine::SessionType::SESSION_TYPE_GROUP == static_cast<IM::BaseDefine::SessionType>(sessionType)) {
    ...
    m_UIRecentConnectedList->UpdateItemInfo(item);
             ...
             static_cast<IM::BaseDefine::SessionType>(sessionType)) {
    module::UserInfoEntity userInfo;
