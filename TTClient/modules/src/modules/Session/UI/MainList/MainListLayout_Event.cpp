...
void MainListLayout::MKOForGrouplistModuleCallback(const std::string& keyId, MKO_TUPLE_PARAM mkoParam) {
  ...
  } else if (module::KEY_GROUPLIST_UPDATE_MYSELF_KICKED == keyId) {  // 自己被踢出了讨论组
  ...
  } else if (module::KEY_GROUPLIST_UPDATE_CREATNEWGROUP == keyId)  // 创建新的讨论组
  {
...
void MainListLayout::MKOForSessionModuleCallback(const std::string& keyId, MKO_TUPLE_PARAM mkoParam) {
  if (module::KEY_SESSION_NEWMESSAGE == keyId) {
    ...
    if (pSessionDialog) {
      MessageEntity msg;
      msg.content =
        util::cStringToString(util::getMultilingual()->getStringById(_T("STRID_SESSIONMODULE_SENDMSG_TOOFAST")));
      msg.sessionId = sId;
      ...
      module::getSessionModule()->asynNotifyObserver(module::KEY_SESSION_NEWMESSAGE,
                                                     msg.sessionId);  // 通知会话模块
    }
  } else if (module::KEY_SESSION_SENDMSG_FAILED == keyId)  // 消息发送失败的提示
  {
    ...
    std::tuple<std::string, UInt16>* pTuple =
      (std::tuple<std::string, UInt16>*)std::get<MKO_SHARED_VOIDPTR>(mkoParam).get();
...
void MainListLayout::MKOForSysConfigModuleCallback(const std::string& keyId, MKO_TUPLE_PARAM mkoParam) {
  if (module::KEY_SYSCONFIG_UPDATED == keyId) {
    ...
    if (!module::getUserListModule()->getUserInfoBySId(sId, userInfo)) {
      ...
      module::getSessionModule()->asynNotifyObserver(module::KEY_SESSION_OPENNEWSESSION,
                                                     sId);  // 通知主窗口创建会话
      return;
    }
    ...
    pFloatWnd->Create(m_pManager->GetPaintWindow(),
                      csTip,
...
void MainListLayout::_CreatSessionDialog(IN UIIMList* pList, IN CControlUI* pMsgSender) {
  ...
  if (-1 != pList->GetItemIndex(pMsgSender) && 0 == _tcsicmp(pMsgSender->GetClass(), _T("ListContainerElementUI"))) {
...
void MainListLayout::Notify(TNotifyUI& msg) {
  if (0 == _tcsicmp(msg.sType, DUI_MSGTYPE_ITEMCLICK)) {
    if (_tcsicmp(msg.pSender->GetClass(), _T("ListContainerElementUI")) == 0) {
      if (m_Tab) {
        if (0 == m_Tab->GetCurSel() && m_EAuserTreelist && -1 != m_EAuserTreelist->GetItemIndex(msg.pSender)) {
          Node* node = (Node*)msg.pSender->GetTag();

          if (m_EAuserTreelist->CanExpand(node)) {
            m_EAuserTreelist->SetChildVisible(node, !node->data().child_visible_);
          }
        } else if (1 == m_Tab->GetCurSel() && m_GroupList && -1 != m_GroupList->GetItemIndex(msg.pSender)) {
          Node* node = (Node*)msg.pSender->GetTag();
...
void MainListLayout::_MySelfKickedFromDiscusGrp(IN const std::string& groupID) {
  ...
  if (module::getGroupListModule()->getGroupInfoBySId(groupID, groupInfo)) {
    ...
    CString text =
      util::getMultilingual()->getStringById(_T("STRID_MAINLISTLAYOUT_KICKEDOUT_MYSELF")) + groupInfo.csName;
