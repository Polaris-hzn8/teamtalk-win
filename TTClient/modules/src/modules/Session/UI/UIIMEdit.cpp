...
MixedMsg::MixedMsg() : m_nSetNetWorkPathSuccTime(0) {}
...
LPVOID UIIMEdit::GetInterface(LPCTSTR pstrName) {
  if (_tcscmp(pstrName, _T("UIIMEdit")) == 0)
    return static_cast<UIIMEdit*>(this);
...
BOOL UIIMEdit::_SaveFile(IN HBITMAP hbitmap, OUT CString& strFilePath) {
  ...
  GetLocalTime(&st);
  CString strTime;
  strTime.Format(_T("%d-%02d-%02d-%02d%02d%02d-%03d"),
                 st.wYear,
                 st.wMonth,
                 st.wDay,
                 st.wHour,
...
void UIIMEdit::InsertImage(BSTR bstrFileName, SIZE size, BOOL isGif) {
  ...
  HRESULT hr = ::CoCreateInstance(
    GifSmiley::CLSID_CGifSmileyCtrl, NULL, CLSCTX_INPROC, GifSmiley::IID_IGifSmileyCtrl, (LPVOID*)&lpAnimator);
...
LRESULT UIIMEdit::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled) {
  if (uMsg == WM_KEYDOWN) {
    ...
    ////fix
    /// bug,当嵌入Windows控件时，如果别的windows控件（如嵌入的IE，它是该窗口的一个子窗口）得到Focus的时候，自己的焦点并没有去掉，造成下次不能再输入
