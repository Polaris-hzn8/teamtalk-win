...
void SessionLayout::_AsynSendReadAck(IN MessageEntity& msg) {
  if (MESSAGE_RENDERTYPE_SYSTEMTIPS == msg.msgRenderType || MESSAGE_RENDERTYPE_FILETRANSFER == msg.msgRenderType) {
    LOG__(APP, _T("not send read ack msgRenderType:%d"), msg.msgRenderType);
    return;
  }
  if (MESSAGE_TYPE_HISTORY == msg.msgStatusType || MESSAGE_TYPE_NONE == msg.msgStatusType) {
...
void SessionLayout::_UpdateGroupMembersList() {
  if (!m_pRightContainer || !m_pGroupMemberList) {
    return;
  }
  module::GroupInfoEntity groupInfo;
  if (!module::getGroupListModule()->getGroupInfoBySId(m_sId, groupInfo)) {
...
void SessionLayout::_AddGroupMemberToList(IN const std::string& sID, IN const BOOL bCreator) {
  ...
  CListContainerElementUI* pListElement = (CListContainerElementUI*)dlgBuilder.Create(
    _T("SessionDialog\\groupMembersListItem.xml"), (UINT)0, NULL, &m_paint_manager);
  if (!pListElement) {
  ...
  for (int n = 0; n < m_pGroupMemberList->GetCount(); n++) {
    ...
    if (pItem) {
      ...
      if (!userData.IsEmpty()) {
        ...
        if (!module::getUserListModule()->getUserInfoBySId(sid, userInfo)) {
          LOG__(
            ERR, _T("can't find the userInfo:%s,GroupId:%s"), util::stringToCString(sid), util::stringToCString(m_sId));
          continue;
          ;
        }
        CButtonUI* btnAvatarInfo = (CButtonUI*)pItem->FindSubControl(_T("AvatarInfo"));
