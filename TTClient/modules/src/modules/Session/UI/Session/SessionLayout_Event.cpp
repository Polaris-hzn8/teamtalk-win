...
void SessionLayout::Notify(TNotifyUI& msg) {
  if (_tcsicmp(msg.sType, DUI_MSGTYPE_WINDOWINIT) == 0) {
    ...
    } else if (msg.pSender == m_pBtnsendfile)  // 文件传输
    {
      ...
      CFileDialog fileDlg(TRUE, NULL, NULL, OFN_HIDEREADONLY | OFN_FILEMUSTEXIST, _T("文件|*.*||"), AfxGetMainWnd());
      fileDlg.m_ofn.Flags |= OFN_NOCHANGEDIR;
      fileDlg.DoModal();
  ...
  *ppchURLOut = 0;
  ...
  } else if (csUrl.Find(_T("moguim/:chat2")) > -1)  // 点击了联系某个人
  {
    ...
    if (-1 != npos) {
      ...
      if (!sUesrID.empty() && sUesrID != module::getSysConfigModule()->userID())  // 不能和自己联系
      {
        // 创建会话窗口
        module::getSessionModule()->asynNotifyObserver(module::KEY_SESSION_OPENNEWSESSION,
                                                       sUesrID);  // 通知主窗口创建会话
...
void SessionLayout::NewWindow2(VARIANT_BOOL*& Cancel, BSTR bstrUrl) {
  *Cancel = VARIANT_TRUE;
  if (m_csTobeTranslateUrl.Find(_T("moguim/:history")) > -1 || m_csTobeTranslateUrl.Find(_T("moguim/:chat2")) > -1 ||
...
void SessionLayout::MKOForGroupModuleCallBack(const std::string& keyId, MKO_TUPLE_PARAM mkoParam) {
  if (keyId == module::KEY_GROUPLIST_UPDATE_SHIELD_SUCCEED)  // 更新群屏蔽按钮
  {
    ...
    module::GroupInfoEntity groupInfo;
    if (module::getGroupListModule()->getGroupInfoBySId(sGroupId, groupInfo) && m_pBtnbanGroupMsg &&
        m_pBtndisplayGroupMsg) {
      bool bShow = false;
    ...
    for (auto picdata : mixedMsgIt->m_picDataVec) {
      ...
      if (SendImgParam::SENDIMG_OK == pParam->m_result) {
        ...
        if (mixedMsgIt->SucceedToGetAllNetWorkPic()) {
          ...
          // 更新会话时间
          module::SessionEntity* pSessionEntity =
            SessionEntityManager::getInstance()->getSessionEntityBySId(msg.sessionId);
          if (pSessionEntity) {
        ...
        MessageEntity msg;
        msg.content =
          util::cStringToString(util::getMultilingual()->getStringById(_T("STRID_SESSIONMODULE_MESSAGE_IMAGEFAILED")));
        msg.sessionId = m_sId;
        msg.talkerSid = module::getSysConfigModule()->userID();
