...
DownloadAvatarHttpOperation::DownloadAvatarHttpOperation(
  std::string sId, std::string& downUrl, BOOL bGrayScale, const std::string format, module::IOperationDelegate callback)
  : IHttpOperation(callback), m_downUrl(downUrl), m_sId(sId), m_bGrayScale(bGrayScale), m_format(format) {}
    ...
    if (m_bGrayScale) {
      ...
      if (bSucc) {
        if (cximage.GrayScale()) {
          CString csGrayPath = module::getMiscModule()->getDownloadDir() + PREFIX_GRAY_AVATAR + csFileName;
