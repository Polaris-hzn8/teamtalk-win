...
CreateDiscussionGrpDialog::CreateDiscussionGrpDialog(const std::string& sid) : m_currentSessionId(sid) {}
...
void CreateDiscussionGrpDialog::OnClick(TNotifyUI& msg) {
  ...
  if (0 == _tcsicmp(msg.pSender->GetName(), _T("okbtn")) && m_pListCreatFrom && m_pListGroupMembers) {
    ...
    if (groupName.IsEmpty()) {
      m_TextErrorTip->SetText(
        util::getMultilingual()->getStringById(_T("STRID_CREATEDISCUSSIONGRPDIALOG_ERROR_EMPTY_GROUPNAME")));
      return;
    }
    if (m_pListGroupMembers->GetCount() < 2) {
      m_TextErrorTip->SetText(
        util::getMultilingual()->getStringById(_T("STRID_CREATEDISCUSSIONGRPDIALOG_ERROR_COUNT")));
      return;
    }
    ...
    imcore::IMLibCoreStartOperationWithLambda([=]() {
      ...
      module::getTcpClientModule()->sendPacket(
        IM::BaseDefine::ServiceID::SID_GROUP, IM::BaseDefine::GroupCmdID::CID_GROUP_CREATE_REQUEST, &imGroupCreateReq);
    });
    Close(IDCANCEL);
