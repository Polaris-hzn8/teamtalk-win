...
BOOL GroupListModule_Impl::popAddedMemberVecBySId(IN const std::string& sID,
                                                  OUT module::UserInfoEntityVec& AddedMemberVec) {
...
BOOL GroupListModule_Impl::popRemovedMemberVecBySId(IN const std::string& sID,
                                                    OUT module::UserInfoEntityVec& RemovedMemberVec) {
...
void GroupListModule_Impl::tcpGetGroupsInfo(IN const module::GroupVec& VecGroupId) {
  ...
  imcore::IMLibCoreStartOperationWithLambda([=]() {
    ...
    module::getTcpClientModule()->sendPacket(
      IM::BaseDefine::ServiceID::SID_GROUP, IM::BaseDefine::GroupCmdID::CID_GROUP_INFO_REQUEST, &imGroupInfoListReq);
  });
}
...
void GroupListModule_Impl::_groupNormalListResponse(IN std::string& body) {
  ...
  for (UInt32 i = 0; i < nSize; ++i) {
    ...
    if (getGroupInfoBySId(groupId, groupinfo)) {
      if (groupinfo.gId == groupId && groupinfo.version == groupVersionInfo.version()) {
        bNeedUpdate = FALSE;
      }
    }
    if (bNeedUpdate) {
...
void GroupListModule_Impl::_groupInfoResponse(IN std::string& body) {
  ...
  if (0 != imGroupInfoListResp.group_info_list_size() && !pGroupIDs->empty()) {
    module::getDatabaseModule()->sqlBatchInsertGroupInfos(m_mapGroups);
    module::getGroupListModule()->asynNotifyObserver(module::KEY_GROUPLIST_UPDATE_GROUPINFO_UPDATE,
                                                     std::shared_ptr<void>(pGroupIDs));
    // 下载所有群的头像
    _downloadAllGroupAvatarImg();
  } else
    LOG__(ERR,
          _T("group_info_list_size is %d,pGroupIDs size:%d"),
          imGroupInfoListResp.group_info_list_size(),
          pGroupIDs->size());
}
...
void GroupListModule_Impl::_groupCreateDiscussGroupResponse(IN std::string& body) {
  ...
  _creatAndSaveSessionToDB(groupInfo);
  module::getGroupListModule()->asynNotifyObserver(module::KEY_GROUPLIST_UPDATE_CREATNEWGROUP,
                                                   groupInfo.gId);  // 通知UI更新讨论组
  // 下载所有群的头像
  _downloadAllGroupAvatarImg();
}
void GroupListModule_Impl::_groupChangedGroupMembersResponse(IN std::string& body) {
  ...
  if (itGroupInfo != m_mapGroups.end()) {
    ...
    if (IM::BaseDefine::GroupModifyType::GROUP_MODIFY_TYPE_DEL == imGroupChangeMemberRsp.change_type()) {
      for (int i = 0; i < imGroupChangeMemberRsp.chg_user_id_list_size(); ++i) {
        ...
        for (std::list<string>::iterator it = groupInfo.groupMemeberList.begin();
             it != groupInfo.groupMemeberList.end();
             ++it) {
          if (*it == sid) {
    ...
    // 通知
    module::getGroupListModule()->asynNotifyObserver(module::KEY_GROUPLIST_UPDATE_MEMBER_CHANGED,
                                                     groupID);  // 通知UI更新
  } else {
...
void GroupListModule_Impl::_groupShieldResponse(IN std::string& body) {
  ...
  if (0 == nRes) {
    ...
    module::getGroupListModule()->asynNotifyObserver(module::KEY_GROUPLIST_UPDATE_SHIELD_SUCCEED,
                                                     groupId);  // 通知UI更新
  } else
...
void GroupListModule_Impl::_groupChangeMemberNotify(IN std::string& body) {
  ...
  if (!getGroupInfoBySId(groupID, groupInfo)) {
    ...
    if (IM::BaseDefine::GroupModifyType::GROUP_MODIFY_TYPE_ADD ==
        imGroupChangeMemberNotify.change_type())  // 只有新增的时候需要去请求
    {
  ...
  } else if (IM::BaseDefine::GroupModifyType::GROUP_MODIFY_TYPE_DEL ==
             imGroupChangeMemberNotify.change_type())  // 删除成员
  {
    std::list<string>& groupMemeberList = m_mapGroups[groupID].groupMemeberList;
    for (int n = 0; n < imGroupChangeMemberNotify.chg_user_id_list_size(); ++n) {
      for (std::list<string>::iterator itUser = groupMemeberList.begin(); itUser != groupMemeberList.end(); ++itUser) {
        std::string sid = util::uint32ToString(imGroupChangeMemberNotify.chg_user_id_list(n));
        bExistMySelf = (sid == module::getSysConfigModule()->userID());
        if (*itUser == sid) {
...
void GroupListModule_Impl::GetSearchGroupNameListByShortName(IN const CString& sShortName,
                                                             OUT module::GroupVec& gidList) {
  ...
  for (auto& kv : m_mapGroups) {
    ...
    if (!group.avatarUrl.empty() && group.avatarUrl.find("avatar_group_default.jpg") == std::string::npos) {
      ...
      if (!imageEntity.filename.empty() && PathFileExists(csLocalPath)) {
        ...
        // 不存在则去服务器下载
        DownloadAvatarHttpOperation* pOper =
          new DownloadAvatarHttpOperation(group.gId,
                                          group.avatarUrl,
                                          TRUE,
                                          AVATAR_FORMAT_48X48,
                                          BIND_CALLBACK_1(GroupListModule_Impl::onCallbackOperation));
...
void GroupListModule_Impl::_safePushBack(IN const std::string& Id, OUT std::list<std::string>& IdList) {
  auto iterID = std::find_if(IdList.begin(), IdList.end(), [=](std::string id) { return id == Id; });
