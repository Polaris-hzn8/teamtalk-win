...
const CString HTML_CHAT2_TAG =
  _T("<a class=\"blue_link\" href=\"moguim://moguim/:chat2?%s\" target=\"_blank\">@%s</a>");  // 替换成连接
...
BOOL ReceiveMsgManage::popMessagesBySId(IN const std::string& sId,
...
UInt32 ReceiveMsgManage::getUnReadMsgCountBySId(const std::string& sId) {
  ...
  if (listChatMsg) {
    ...
    int nUnReadCount = 0;
    for (MessageEntity msg : msgList) {  // 若是自己发的消息，则是多端同步的，不需要统计到未读计数中
...
void ReceiveMsgManage::parseContent(CString& content, BOOL bFloatForm, Int32 chatWidth) {
  ...
  while (startIndex != -1) {
    ...
    if (-1 == endIndex)
      break;
    CString csImgUrlPath = content.Mid(startIndex + CS_SPLIT_CODE_START.GetLength(),
                                       endIndex - startIndex - CS_SPLIT_CODE_START.GetLength());
...
BOOL ReceiveMsgManage::checkIsReduplicatedMsg(IN const MessageEntity& msg, IN const UInt32 seqNo) {
  ...
  if (itMap != m_MsgMap.end()) {
    ...
    BOOL bFind = FALSE;
    for (ReceiveMsgList::reverse_iterator itList = MsgList.rbegin(); itList != MsgList.rend(); ++itList) {
      if (itList->msg.content == msg.content && itList->seqNo == seqNo) {
        bFind = TRUE;
        return TRUE;
      }
    }
...
void ReceiveMsgManage::_urlScan(CString& content) {
  const std::wregex pattern(
    _T("((http[s]{0,1}|ftp)://[a-zA-Z0-9\\.\\-]+\\.([a-zA-Z]{2,4})(:\\d+)?(/[a-zA-Z0-9\\.\\-~!@#$%^&*+?:_/")
    _T("=<>]*)?)|(www.[a-zA-Z0-9\\.\\-]+\\.([a-zA-Z]{2,4})(:\\d+)?(/[a-zA-Z0-9\\.\\-~!@#$%^&*+?:_/=<>]*)?)"));
  std::wsmatch what;
...
BOOL AudioMessageMananger::playAudioMsgByAudioSid(IN const std::string& sSessionID, IN const std::string& sAID) {
  ...
  if (itAudio != m_mapUnReadAudioMsg.end()) {
    ...
    for (SessionMessage_List::iterator it = listChatMsg.begin(); it != listChatMsg.end(); ++it) {
      if (it->content == sAID) {
        it->msgAudioReaded = 1;
      }
    }
...
BOOL AudioMessageMananger::makeAppAudioSid(IN const UInt32 msgId,
                                           IN const std::string sSessionId,
                                           OUT std::string& sAID) {
  // CString strTime;
