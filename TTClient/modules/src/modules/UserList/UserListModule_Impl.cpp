...
void UserListModule_Impl::_allUserlistResponse(IN std::string& pbBody) {
  ...
  LOG__(
    APP, _T("IMAllUserRsp,latest_update_time = %d,Local_update_time = %d"), imAllUserRsp.latest_update_time(), time);
  if (imAllUserRsp.latest_update_time() != time)  // 比较latest_update_time
  {
    ...
    module::getSysConfigModule()->saveUserInfoLatestUpdateTime(
...
void UserListModule_Impl::_removeSessionNotify(IN std::string& pbBody) {
  ...
  if (IM::BaseDefine::SessionType::SESSION_TYPE_GROUP == imRemoveSessionNotify.session_type()) {
    ...
    LOG__(ERR, _T("Unkonwn session_type:%d, sessionId:%d"), imRemoveSessionNotify.session_type(), id);
  }
...
void UserListModule_Impl::_departmentResponse(IN std::string& pbBody) {
  ...
  LOG__(APP,
        _T("IMDepartmentRsp,latest_update_time = %d,Local_update_time = %d"),
        imDepartmentRsp.latest_update_time(),
        time);
...
void UserListModule_Impl::_recentlistResponse(IN std::string& pbBody)  // 最近联系人群信息
{
  ...
  for (int i = 0; i < nSize; i++) {
    ...
    LOG__(DEBG,
          _T("sessionID = %s,updatedTime = %d"),
          util::stringToCString(recentSessionInfo.sessionID),
          recentSessionInfo.updatedTime);
...
void UserListModule_Impl::_userStatusNotify(IN std::string& pbBody) {
  ...
  std::string sid = util::uint32ToString(userStat.user_id());
  LOG__(DEBG,
        _T("IMUserStatNotify,sid=%s,status=%d"),
        util::stringToCString(sid),
...
void UserListModule_Impl::tcpGetUsersInfo(IN const module::UserInfoEntityVec& VecUnKnowUserInfo) {
  ...
  //		std::vector<std::string> VecId(nLeft);
  //		std::vector<std::string>::iterator itSend = VecId.begin();
  //		std::copy(it + m_tcpGetUserFriendInfoListTime*nMax, it + m_tcpGetUserFriendInfoListTime*nMax + nLeft,
  // itSend); 		_tcpGetUserInfoList(VecId);
  //	}
  // }
  // else
  //{
...
void UserListModule_Impl::getSearchUserNameListByShortName(IN const CString& sShortName,
                                                           OUT module::UserInfoEntityVec& nameList) {
  for (auto& kvp : m_mapUsers) {
...
BOOL UserListModule_Impl::_downloadAvatarImgBySId(IN const std::string& sId) {
  ...
  if (!user.avatarUrl.empty() && user.avatarUrl.find("avatar_default.jpg") == std::string::npos) {
    ...
    if (!imageEntity.filename.empty() && PathFileExists(csLocalPath)) {
      ...
      // 不存在则去服务器下载
      DownloadAvatarHttpOperation* pOper = new DownloadAvatarHttpOperation(
        user.sId, user.avatarUrl, TRUE, strFormat, BIND_CALLBACK_1(UserListModule_Impl::onCallbackOperation));
...
void UserListModule_Impl::onCallbackOperation(std::shared_ptr<void> param) {
  ...
  if (DownloadImgParam::DOWNLOADIMG_OK == pDownParam->m_result) {
    ...
    if (userIter != m_mapUsers.end()) {
      CString csLocalPath =
        module::getMiscModule()->getDownloadDir() + util::stringToCString(pDownParam->m_imgEntity.filename);
    ...
    *pTuple = std::tie(module::MODULE_USERLIST_PREFIX, pDownParam->m_sId);
    module::getUserListModule()->asynNotifyObserver(module::KEY_USERLIST_DOWNAVATAR_SUCC,
                                                    std::shared_ptr<void>(pTuple));
  }
}
...
void UserListModule_Impl::_changeSignInfoResponse(IN std::string& pbBody) {
  ...
  LOG__(
    APP, _T("IMChangeSignInfoRsp,sid = %s,SignInfo=%s"), util::stringToCString(sid), util::stringToCString(sSignInfo));
...
void UserListModule_Impl::_avatarChangeNotify(IN std::string& pbBody)  // 修改头像签名通知
{
  ...
  LOG__(APP,
        _T("IMAvatarChangedNotify,sid = %s,sAvatarUrl=%s"),
        util::stringToCString(sid),
        util::stringToCString(sAvatarUrl));
  module::UserInfoEntity userInfo;
  if (!getUserInfoBySId(sid, userInfo)) {
    LOG__(DEBG, _T("Can't find the sid"));
    return;
...
void UserListModule_Impl::_signInfoChangedNotify(IN std::string& pbBody)  // 修改个性签名通知
{
  ...
  LOG__(APP,
        _T("IMSignInfoChangedNotify,sid = %s,sSignInfo=%s"),
        util::stringToCString(sid),
        util::stringToCString(sSignInfo));
  module::UserInfoEntity userInfo;
  if (!getUserInfoBySId(sid, userInfo)) {
    LOG__(DEBG, _T("Can't find the sid"));
    return;
