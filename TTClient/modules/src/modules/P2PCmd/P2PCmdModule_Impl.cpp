...
#include <modules/P2PCmd/P2PCmdModule_Impl.h>
...
void P2PCmdModule_Impl::_p2pCmdNotifyResponse(std::string& pbBody) {
  ...
  LOG__(APP,
        _T("IMP2PCmdMsg cmd_msg_data = %s,sFromId = %s"),
        util::stringToCString(jsonMsgData),
        util::stringToCString(sFromId));
  UINT32 nServiceID = 0;
  UINT32 nCmdID = 0;
...
void P2PCmdModule_Impl::_makeJsonData(IN UINT32 nServiceID,
                                      IN UINT32 nCmdID,
                                      IN std::string sContent,
                                      OUT std::string& sJsonData) {
  Json::Value root;
