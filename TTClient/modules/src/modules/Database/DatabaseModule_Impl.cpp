...
DatabaseModule_Impl::DatabaseModule_Impl() : m_pSqliteDB(new CppSQLite3DB()), m_pSqliteGlobalDB(new CppSQLite3DB()) {
  _startup();
}
...
BOOL DatabaseModule_Impl::_openDB() {
  ...
  try {
    ...
    LOG__(ERR, _T("open database failed,error msg:%s"), csErrMsg);
    // #ifdef _DEBUG
...
void DatabaseModule_Impl::_closeDB() {
  try {
    ...
    CString csErrMsg = util::stringToCString(sqliteException.errorMessage());
    LOG__(ERR, _T("close database failed,error msg:%s"), csErrMsg);
#ifdef _DEBUG
    MessageBoxA(0, sqliteException.errorMessage(), "BD ERROR", MB_OK | MB_ICONHAND);
#endif
  } catch (...) {
