...
const std::string createImImageSql =
  ...
  ");";
const std::string createImImageIndex = "CREATE unique INDEX IF NOT EXISTS hashcode_idx ON imimage(hashcode);";
const std::string insertImImageSql =
  "INSERT OR REPLACE INTO imimage(hashcode,localPath,urlPath) "
  "VALUES(?, ? ,? );";
const std::string getImImageByHashcodeSql = "select * from imimage where hashcode=? limit 1";
const std::string updateImImageByHashcodeSql = "update imimage set localPath=? where hashcode=?";
}  // namespace
...
#endif
    CString csErrMsg = util::stringToCString(sqliteException.errorMessage(), CP_UTF8);
    LOG__(ERR, _T("db sqlGetImImageEntityByHashcode failed,error msg:%s"), csErrMsg);
    return FALSE;
  } catch (...) {
    LOG__(ERR, _T("db unknown exception"));
    return FALSE;
