...
const std::string createUserInfoSql =
  ...
  ");";
const std::string createUserInfoIndex = "create unique index if not exists userid_idx ON userinfo(userId);";
const std::string getAllUsersInfoSql = "select * from userinfo where status<>3 limit 10000";
const std::string getUserInfoBySIdSql = "select * from userinfo where userId=? limit 1";
const std::string insertUserInfoSql =
  "INSERT OR REPLACE into "
  "userinfo(userId,name,nickName,avatarUrl,departmentId,departmentName,email,"
  "gender,user_domain,telephone,status,reserve1) "
  "VALUES(?,?,?,?,?,?,?,?,?,?,?,?);";
const std::string updateUserInfoBySIdSql =
  "update userinfo set "
  "name=?,nickName=?,avatarUrl=?,departmentId=?,departmentName=?,email=?,"
  "gender=?,user_domain=?,telephone=?,status=?,reserve1=? where userId=?";
