...
const std::string createDepartmentInfoSql =
  ...
  ");";
const std::string createDepartmentInfoIndex = "create unique index if not exists dId_idx ON departmentinfo(dId);";
const std::string getAllDepartmentInfoSql = "select * from departmentinfo where status=0 limit 10000";
const std::string getDepartmentInfoBySIdSql = "select * from departmentinfo where dId=? limit 1";
...
BOOL DatabaseModule_Impl::sqlUpdateDepartmentInfoEntity(std::string& sId,
                                                        IN const module::DepartmentEntity& departmentInfo) {
  try {
