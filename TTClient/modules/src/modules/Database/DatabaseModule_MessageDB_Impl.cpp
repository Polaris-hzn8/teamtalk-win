...
const std::string insertMessageSql =
  ...
  "immessage(msgId,sessionId,talkerid,content,rendertype,sessiontype,msgtime,createtime,reserve1,reserve2,reserve3) "
  "VALUES(?,?,?,?,?,?,?,?,?,?,?);";
...
BOOL DatabaseModule_Impl::sqlGetHistoryMessage(IN const std::string& sId,
                                               IN const UInt32 msgId,
                                               IN UInt32 nMsgCount,
  ...
  try {
    ...
    while (!query.eof()) {
      ...
      // msgList.insert(msgList.begin(), msg);//???
      query.nextRow();  // msgList.insert(msgList.begin(), msg);//需要
    }
    ...
    LOG__(ERR, _T("get history message failed,error msg:%s"), csErrMsg);
    return FALSE;
  } catch (...) {
...
BOOL DatabaseModule_Impl::sqlInsertMessage(IN MessageEntity& msg) {
  ...
  if (msg.msgId <= 0) {
    ...
    LOG__(ERR,
          _T("msgid <= 0, msgid:%d msg_content:%s Don't save to DB!"),
          msg.msgId,
          util::stringToCString(msgDecrptyCnt));
    return FALSE;
  }
