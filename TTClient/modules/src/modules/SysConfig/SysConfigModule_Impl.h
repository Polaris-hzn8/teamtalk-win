...
/*
 ...
 brief: 系统配置信息管理，包括系统设置和安全证书配置信息
*/
...
class SysConfigModule_Impl final : public module::ISysConfigModule {
 ...
 public:
  ...
  virtual UInt32 getUserInfoLatestUpdateTime(void);  // 获得用户更新时间
  ...
  // 图片获取逻辑
  virtual BOOL getImage(IN std::string sid,
                        IN std::string url,
                        IN BOOL bGrayScale,
 ...
 private:
  ...
  BOOL m_bSysConfigDialogFlag;  // 确定窗口是否实例化
};
