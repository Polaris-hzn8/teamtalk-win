...
AutoSelectMode::AutoSelectMode() : m_hPrevMouseWindow(NULL), m_iPrevMouseXPos(-1), m_iPrevMouseYPos(-1) {
  m_lsWndList.clear();
}
...
ScreenCaptureWndInfo* AutoSelectMode::windowFromPoint(__in int x, __in int y) {
  std::vector<ScreenCaptureWndInfo>::iterator itr =
    std::find(m_lsWndList.begin(), m_lsWndList.end(), ScreenCaptureWndInfo(x, y));
