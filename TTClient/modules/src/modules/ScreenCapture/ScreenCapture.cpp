...
#include <modules/ScreenCapture/UI/UIMgr.h>
...
BOOL ScreenCapture::createMsgWindow() {
  ...
  return (NULL != CreateWindow(szWndName, szWndName, 0, 0, 0, 0, 0, HWND_MESSAGE, NULL, NULL, NULL));
}
...
BOOL ScreenCapture::startCapture(__in std::wstring strSavePath,
                                 __in ScreenCaptureCallback* captureCallback /* = NULL*/,
...
void ScreenCapture::finishCapture() {
  ...
  cancelCapture();
  if (m_callBack)
    m_callBack->onScreenCaptureFinish(m_strSavePath);
}
