...
// KEYID
const std::string KEY_USERLIST_UPDATE_DEPARTMENTLIST =
  MODULE_USERLIST_PREFIX + "DeparmentList";                                                    // 成功获取到组织架构信息
...
const std::string KEY_USERLIST_ALLUSERLINESTATE =
  MODULE_USERLIST_PREFIX + "AllUserLineState";                                            // 列表所有用户在线状态通知
const std::string KEY_USERLIST_USERLINESTATE = MODULE_USERLIST_PREFIX + "UserLineState";  // 单个用户在线状态通知
const std::string KEY_USERLIST_USERSIGNINFO_CHANGED =
  MODULE_USERLIST_PREFIX + "UserSignInfoChanged";  // 用户个性签名更变通知
...
class MODULE_CLASS UserInfoEntity {
 ...
 public:
  UInt8 onlineState = IM::BaseDefine::USER_STATUS_OFFLINE;  // 用户在线状态  	USER_STATUS_ONLINE 	=
                                                            // 1,USER_STATUS_OFFLINE	= 2,USER_STATUS_LEAVE	= 3,
...
// 组织架构相关模块
class MODULE_API IUserListModule : public module::ModuleBase, public module::IPduPacketParse {
 public:
  /**
   * 用户模块初始化处理（加载本地用户数据）
   *
   ...
   * @exception there is no any exception to throw.
   */
  virtual void getSearchUserNameListByShortName(IN const CString& sShortName,
                                                OUT module::UserInfoEntityVec& nameList) = 0;
  virtual void getSearchDepartmentsByShortName(IN const CString& sShortName,
                                               OUT std::vector<std::string>& departmentList) = 0;
