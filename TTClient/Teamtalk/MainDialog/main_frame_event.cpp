...
void MainFrame::OnClick(TNotifyUI& msg) {
  ...
  } else if (0 == _tcsicmp(msg.pSender->GetName(), _T("ToolsBtn")))  // 工具按钮
  {
  ...
  {
    FloatWnd* pFloatWnd = new FloatWnd();  // 安全中心

    if (pFloatWnd == NULL)
      return;
    pFloatWnd->Create(m_hWnd,
                      _T("FloatWnd"),
                      UI_WNDSTYLE_FRAME | WS_THICKFRAME,
                      WS_EX_TOOLWINDOW | WS_EX_TOPMOST | WS_EX_STATICEDGE,
                      0,
                      0,
                      0,
                      0);
    ...
    if (pLoginWnd == NULL)
      return;
    pLoginWnd->Create(NULL,
                      _T("LoginWnd"),
                      /*WS_POPUP | WS_VISIBLE | WS_CLIPSIBLINGS*/ UI_WNDSTYLE_FRAME | WS_POPUP,
                      0 /*WS_EX_LAYERED*/,
                      0,
                      0,
                      0,
      ...
      if ((pFriendsList != NULL) && pFriendsList->GetItemIndex(msg.pSender) != -1) {
        if (_tcsicmp(msg.pSender->GetClass(), _T("ListContainerElementUI")) == 0) {
          ...
          if (!pFriendsList->CanExpand(node) && (background != NULL)) {
            ...
            for (std::vector<FriendListItemInfo>::const_iterator citer = friends_.begin(); citer != friends_.end();
                 ++citer) {
