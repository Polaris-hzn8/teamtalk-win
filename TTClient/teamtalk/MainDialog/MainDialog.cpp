...
void MainDialog::InitWindow() {
  ...
  SetWindowPos(
    m_hWnd, HWND_TOPMOST, rcWork.GetWidth() - nWidth - 100, 100, 280, rcWork.GetHeight() - 200, SWP_SHOWWINDOW);
}
...
void MainDialog::UpdateTotalUnReadMsgCount(void) {
...
void MainDialog::FreshMySignature(void) {
...
void MainDialog::OnHotkey(__in WPARAM wParam, __in LPARAM lParam) {
  ...
  if (module::SC_HK_START_CAPTURE == emHotkeyId) {
    ...
    CString strFileName;
    strFileName.Format(
      _T("%4d%02d%02d%02d%02d%02d.bmp"), tm.wYear, tm.wMonth, tm.wDay, tm.wHour, tm.wMinute, tm.wSecond);
...
void MainDialog::OnClick(TNotifyUI& msg) {
  ...
  if (msg.pSender == m_pbtnSysConfig) {
    ...
    module::getSysConfigModule()->asynNotifyObserver(module::KEY_SYSCONFIG_SHOW_USERDETAILDIALOG,
                                                     module::getSysConfigModule()->userID());
  } else if (msg.pSender == m_pbtnClose || msg.pSender == m_pbtnMinMize) {
    ShowWindow(false);
    return;
  }
  __super::OnClick(msg);
