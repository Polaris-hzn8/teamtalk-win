...
#include <imcore\base\BaseSocket.h>
#include <imcore\base\EventDispatch.h>
...
void CBaseSocket::_SetAddr(const char* ip, const uint16_t port, sockaddr_in* pAddr) {
  ...
  // Q1:使用过时的网络函数 inet_addr 是用于将IP地址转换为网络字节序的32位无符号整数
  pAddr->sin_addr.s_addr = inet_addr(ip);

  if (pAddr->sin_addr.s_addr == INADDR_NONE) {
    // Q2:使用过时的网络函数 gethostbyname 通过ip地址获取主机名
