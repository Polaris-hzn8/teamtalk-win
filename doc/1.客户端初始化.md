...
1. 日志库初始化：`_InitLog()` 
2. 启动网络通信线程：`IMLibCoreRunEvent()` 
   - 检测网络数据读写，再启动线程创建一个队列，如果队列中有任务，则取出该任务执行

3. 启动UI事件循环：`module::getEventManager()->startup()` 
   - 创建子线程与UI线程的桥梁-代理窗口

4. 创建用户文件夹
5. DuiLib初始化：配置duilib的资源文件路径、初始化com库、初始化ole库 
6. 显示登录服务器信息配置窗口：如果没有配置登录服务器的地址，则显示配置对话框
7. 显示登录窗口：显示登录对话框
8. 主应用窗口：登录成功后，登录对话框销毁，显示主对话框
9. 启动duilib的消息循环（不使用mfc的消息循环）

上述是teamtalk启动的大致流程，下面就详细分析下这些启动流程

### 1.启动操作处理线程

IMLibCoreRunEvent中调用 getOperationManager()->startup();来启动逻辑任务队列线程，代码如下：
...
io线程启动后运行event_run函数，在这个函数里启动事件分发函数StartDispatch，Windows环境下使用select对socket句柄的事件进行监听，Linux环境下的服务端使用epoll对socket句柄的事件进行监听。总体大致的流程就是循环超时等待监听的socket是否有事件发生，有的话就进行对应的事件处理。
...
这里duilib的消息响应机制，类似qt的qml进行界面和后台逻辑分离，使用方便，

teamtalk中的界面类继承 INotifyUI，重写Notify函数进行界面消息的响应（具体使用可以自行查看相关资料），DuiLib消息响应方式：https://blog.csdn.net/kerwinash/article/details/38556069
...
3. 添加消息代理（其实就是将成员函数最为回到函数加入），MakeDelegate(this, &CFrameWindowWnd::OnAlphaChanged); 当程序某个地方调用了CPaintManagerUI::SendNotify，并且Msg.pSender正好是this，类成员回调函数将被调用
